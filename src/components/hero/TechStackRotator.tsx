...
export default function TechStackRotator({
  onTechClick,
}: TechStackRotatorProps) {
    ...
    const preloadImages = async () => {
      const imagePromises = techStack.map((tech) => {
        return new Promise((resolve, reject) => {
          ...
          img.onerror = () =>
            reject(new Error(`Failed to load ${tech.name} logo`));
          img.src = tech.logo;
        });
      });
  ...
  if (!imagesLoaded) {
    ...
    return (
      <div className="flex items-center justify-center lg:justify-start gap-3 mb-4">
        ...
        <div
          className={`px-4 py-2 rounded-lg bg-gradient-to-r ${techStack[0].color} text-white font-bold text-lg flex items-center gap-2`}
        >
    ...
    <div className="flex items-center justify-center lg:justify-start gap-3 mb-4">
      ...
      <button
        ...
        style={{
          ...
          transition:
            "width 200ms ease-in-out, background 500ms ease-in-out, transform 300ms ease-in-out, box-shadow 300ms ease-in-out",
        }}
      >
